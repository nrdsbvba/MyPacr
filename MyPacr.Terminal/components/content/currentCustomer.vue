    ...
    <div v-else>
      <div v-if="currentCustomer">
        ...
        <div class="text-center">
          {{ currentCustomer.card_leasing_type.name }}
        </div>
        <hr />
        <b-container class="bv-example-row">
          <b-row>
            <b-col>
              <img
                :src="createArticleThumbnail(currentCustomer.user.picture)"
                class="float-left"
            /></b-col>
            ...
            <b-col
              ><h4
                class=" m-3 align-top"
                :class="{
                  'text-danger': parseFloat(currentCustomer.saldo) <= 0,
                  'text-success': parseFloat(currentCustomer.saldo) < 0
                }"
              >
              ...
              <div>
                <span class=" m-3 align-top"
                  >{{ usergroupArrayToString(currentCustomer.user.usergroup) }}
                </span>
              </div>
            </b-col>
          </b-row>
      ...
      <div v-if="currentCustomer">
        <b-button-group size="lg">
          <b-button
            ...
            class="btn-rounded-left text-center"
            size="lg"
          ...
          <b-button
            ...
            class="btn-rounded-right text-center"
            size="lg"
            @click="showTransactions"
          >
            <i class="fas fa-list-ul fa-lg" /> Transacties</b-button
          >
        </b-button-group>
      </div>
    </div>
    <b-modal id="transactionModal" size="xl" title="Transacties">
      <b-table
        :items="currentCustomer ? currentCustomer.transactions : []"
        responsive="sm"
        :fields="transactionFields"
        :tbody-tr-class="rowClass"
      >
        <template slot="[return]" slot-scope="row">
...
export default {
  ...
  computed: {
    currentCustomer: {
      get() {
        var tmp = this.$store.state.currentCustomer
        return tmp
      },
      set() {
        //
  ...
  methods: {
    ...
    getUserByCard: function(cardId) {
      tobackend.getUserCardByCardId(this.$store, cardId).then((resp) => {
        //wait for complete
        this.spinner = false
        if (!resp) {
          this.$bvToast.toast(
            'Kan gebruiker met cardId: ' + cardId + ' niet vinden',
            {
              title: 'Gebruiker zoeken',
              autoHideDelay: 5000,
              appendToast: false,
              variant: 'danger'
            }
    ...
    createArticleThumbnail: function(filename) {
      if (!filename) {
        ...
        return tobackend.getThumbnail(filename.filename_download, 300, 150)
        // , 'contain'
      }
    },
    ...
    clearCurrentUser: function() {
      ...
      this.$store.commit('setCurrentCustomerEntryFeePayedToday', null),
        this.$store.commit('setActivePriceLevel', null)
      this.$store.commit('recalculateShoppingCart', true)
    },
    showTransactions: function() {
      //do stuff to show transactions
    },
